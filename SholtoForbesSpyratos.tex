...
After pitchover the angle of attack reduces gradually until in stages in order to reach the desired end conditions. 
...
A constant dynamic pressure trajectory with a pull-up to 1.5$^\circ$ trajectory angle at third stage release is produced as a baseline for comparison with a payload-optimised trajectory, and to verify that LODESTAR is able to optimise a complex airbreathing trajectory. Due to the clear objective of a constant dynamic pressure trajectory, any deviations from the target dynamic pressure are readily apparent, allowing the efficacy of the optimiser to be verified. A pull-up to 1.5$^\circ$ trajectory angle is approximately the minimum necessary for the third stage to reach orbit. At release angles below 1.5$^\circ$ the thrust vector limitations necessary to produce a trimmed trajectory cause the third stage to not be able to exit the atmosphere. 
 ...
 These results show very close adherence to 50kPa dynamic pressure (maximum 0.04\% deviation) until pull-up at 336.4s. Third stage release occurs at \FlightTimeConstq s at \SeparationAltConstq km altitude. 
...
The initial part of the third stage trajectory released at 1.5$^\circ$, shown in Figure \ref{fig:ThirdStageConstQ}, involves a decrease in trajectory angle prior to the pull-up. At the beginning of this trajectory, the rocket is limited by the maximum thrust vector angle and does not have the lift required to sustain increasing altitude. 
The trajectory angle initially decreases as the third stage accelerates in-atmosphere at close to the thrust vector limit. The increased lift gained as the velocity increases then enables the third stage to reach exoatmospheric conditions. 

The release of the third stage rocket from an optimised scramjet trajectory is shown in Figure \ref{fig:ThirdStage50kPa}. Release at a higher, more optimal angle allows the effects of the thrust vector angle restriction to be decreased. The rocket increases trajectory angle and gains altitude rapidly, resulting in less time spent in-atmosphere, and a larger payload to orbit. 
