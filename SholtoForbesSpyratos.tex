...
The trajectory profile of a rocket-scramjet-rocket system utilising the SPARTAN scramjet accelerator and a third stage rocket powered by a Pratt \& Whitney RL-10A-3A Engine, has previously been designed around the scramjet stage flying a constant dynamic pressure trajectory\cite{Preller2017}, in-line with similar multi-stage vehicles\cite{Kimura1999,Olds1998}.
 A constant dynamic pressure trajectory follows the maximum dynamic pressure that the scramjet-powered vehicle is able to withstand structurally, ensuring maximum thrust production from the scramjet engines. However, this results in the release of the third stage rocket into a high dynamic pressure trajectory, at low trajectory angle. Release at these conditions requires sufficient thrust for the third stage rocket to accelerate under the effects of the high drag produced by the maximum dynamic pressure flight conditions.  
 
 This study utilises a modified SpaceX Kestrel engine\cite{Vehicle2008} as the propulsion system for the third stage rocket, rather than the Pratt \& Whitney RL-10A-3A Engine, which has been used previously in launch systems involving the SPARTAN scramjet accelerator\cite{Preller2017}. The Kestrel engine is a pressure-fed engine, which has been chosen for cost-efficiency, while trading off for decreased thrust and efficiency compared to the RL-10A-3A. This reduced thrust causes the rocket to not be able to reach orbit after release at the end of a constant dynamic pressure trajectory, necessitating release at a lower-than-maximum dynamic pressure. To lower the dynamic pressure, the scramjet stage must perform a pull-up manoeuvre before third stage separation. 
 
 A minimum pull-up trajectory to the point which allows the third stage rocket to reach orbit will produce maximum acceleration from the scramjet stage but may not be optimal for the overall multi-stage system. 
 For example, a constant dynamic pressure trajectory with minimum pull-up may still release the final stage into a high dynamic pressure environment, at a potentially suboptimal trajectory angle. This release point may result in suboptimal performance of the final stage rocket  and the inclusion of unnecessary design constraints. Calculating a trajectory using optimal control methods allows for a trade-off between the efficiency of the stages, to produce a maximum payload trajectory that does not necessarily involve the scramjet stage flying at maximum dynamic pressure for its entire flight.
 ...
 The trajectories of launch vehicles utilising rocket and airbreathing propulsion within a single stage have previously been analysed using optimal control\cite{Powell1991,Lu1993,Trefny1999}. The optimised trajectories for these single stage vehicles show flight at maximum dynamic pressure for the majority of airbreathing operation, followed by a pull-up before airbreathing engine cut-off\cite{Powell1991,Lu1993,Trefny1999}. 
...
The third stage trajectory utilises the direct single shooting method, the simplest form of direct optimal control\cite{Diehl2006}. The direct single shooting method involves discretising the controls of the system, and simulating the trajectory as a forward simulation\cite{Diehl2006}. The direct single shooting method allows a problem to be solved from only a guess of the controls, enabling the initial conditions of the optimal control problem to vary without significant changes to the guess or bounds of the problem. The direct single shooting method is widely used in engineering applications due to its ease of application to a wide variety of problems\cite{Diehl2006}.
...
An altitude of 24.4km is reached after a total flight time of 94.0s. 
